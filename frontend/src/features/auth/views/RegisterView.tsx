...
export const RegisterView: React.FC<RegisterViewProps> = ({
  ...
  onRegisterSuccess = (_registeredPhone: string, email: string) => {
    window.location.hash = `#/verify-otp?email=${encodeURIComponent(email)}&flow=register`;
  }
}) => {
  const { users } = useApp();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!passwordRegex.test(password)) {
      setErrorMsg(
        'Mật khẩu phải có độ dài tối thiểu 8 ký tự và bao gồm ít nhất 1 chữ cái viết hoa, 1 chữ cái viết thường, 1 chữ số và 1 ký tự đặc biệt.'
      );
      return;
    }
    ...
    if (USE_REAL_BACKEND) {
      try {
        await authApi.register(fullname.trim(), email.trim(), cleanPhone, password);
    ...
    users.push(newUser);
    setSuccessMsg(
      'Đăng ký tài khoản thành công! Mã OTP xác thực (giả lập) đã được gửi tới địa chỉ email của bạn. Vui lòng sử dụng mã 123456 để xác thực.'
    );
    setLoading(false);
    ...
    <div className="min-h-screen bg-[#f5f5f5] flex items-center justify-center px-4 py-8 text-left font-body-md">
      <div className="bg-white rounded-3xl border border-gray-200/80 shadow-sm w-full max-w-md px-8 py-10 space-y-6">
        ...
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <div className="text-center space-y-1">
            ...
            <p className="text-sm text-gray-500 font-medium">
              Bắt đầu hành trình kết nối và cống hiến vì cộng đồng
            </p>
          </div>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          {/* Số điện thoại Input */}
          <div className="space-y-1">
            <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider" htmlFor="phone">
              Số điện thoại
            </label>
            <div className="relative">
              <span
