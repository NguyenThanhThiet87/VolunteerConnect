...
export const RequestOrganizerView: React.FC = () => {
  const { currentUser, submitOrganizerRequest, showNotification, organizerRequests } =
    useApp();
  ...
  const handleSendRequest = async (e: React.FormEvent) => {
    ...
    if (requestOrgDesc.trim().length < 10 || requestOrgDesc.trim().length > 500) {
      showNotification(
        'Lý do muốn trở thành Nhà tổ chức phải từ 10 đến 500 ký tự.',
        'error'
      );
      return;
    }
        ...
        <div className="max-w-2xl mx-auto bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-4 sm:p-8 md:p-10 shadow-sm space-y-6">
          <div className="space-y-1">
            <span className="text-xs font-bold text-slate-400">
              Hồ sơ cá nhân &gt; Xin quyền tổ chức
            </span>
          ...
          {isPending || inCooldown ? (
            <div
              className={`p-5 rounded-2xl border ${
                isPending
                  ? 'bg-[#fef7e0] border-[#b06000]/30 text-[#b06000]'
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-base font-bold">
                  {isPending
                    ...
                    : 'Bạn đang trong thời gian chờ (Cooldown)'}
                </h3>
              </div>
              <p>
                {isPending
              ...
              <div className="mt-4 text-center md:text-left">
                <a
                  href="#/profile"
                  className="inline-block bg-[#006d37] hover:bg-emerald-800 text-white font-bold py-2 px-6 rounded-xl transition-all shadow-sm"
                >
                  Quay về Hồ sơ
                </a>
              </div>
            ...
            <form onSubmit={handleSendRequest} className="space-y-6 pt-2">
              ...
              {/* Kinh nghiệm / Tên tổ chức */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                  Kinh nghiệm hoạt động / Tên tổ chức đại diện
                </label>
                <textarea
                  rows={4}
