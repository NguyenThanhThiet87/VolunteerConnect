...
import { formatISOToLocalInput, formatDateTimeToISO } from '../../../core/utils/formatters';
import { mediaApi } from '../../posts/api/postApi';

interface EditActivityModalProps {
  activity: Activity;
  onClose: () => void;
  onSubmit: (activityId: string, activityData: Partial<Activity>) => Promise<void>;
}
...
export const EditActivityModal: React.FC<EditActivityModalProps> = ({
  ...
  const [selectedCategory, setSelectedCategory] = useState(
    activity.categories[0] || ACTIVITY_CATEGORIES[0]
  );
  const [startDate, setStartDate] = useState(formatISOToLocalInput(activity.start_date));
  const [endDate, setEndDate] = useState(formatISOToLocalInput(activity.end_date));
  const [limitVolunteers, setLimitVolunteers] = useState<number>(
    activity.limit_volunteers || 10
  );
  const [province, setProvince] = useState(
    activity.location?.province || PROVINCE_OPTIONS[0]
  );
  const [district, setDistrict] = useState(
    activity.location?.district || LOCATION_DATA[PROVINCE_OPTIONS[0]]?.[0] || ''
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      showNotification(err.message || 'Không thể cập nhật hoạt động.', 'error');
    } finally {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-3 sm:p-4 animate-fadeIn">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-200 overflow-y-auto max-h-[92dvh] sm:max-h-[90vh] animate-scaleUp text-left">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h3 className="font-bold text-gray-900 text-xl">Chỉnh sửa thông tin hoạt động</h3>
          <button
            onClick={onClose}
